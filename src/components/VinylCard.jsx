export default function VinylCard({ isPlaying, currentTrack }) {
    return (
        <div className="bg-white/10 backdrop-blur-xs border border-white rounded-[4rem] shadow-[0_20px_100px_-20px_rgba(0,0,0,10)] p-6 md:p-8 relative flex items-center justify-center aspect-square w-full sm:w-[320px] md:w-[380px]">
            ...
            <div className={`w-[85%] h-[85%] rounded-full bg-[#1c1c1c] shadow-[0_15px_30px_rgba(0,0,0,0.2)] flex items-center justify-center p-2 border-[6px] border-[#222] pointer-events-none ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
                <div className="w-full h-full rounded-full border border-[#2a2a2a] flex items-center justify-center" style={{ backgroundImage: 'repeating-radial-gradient(#1c1c1c 0px, #1c1c1c 2px, #222 3px, #222 5px)' }}>
                    <div className="w-[35%] h-[35%] bg-white rounded-full flex flex-col items-center justify-center shadow-inner relative z-10 overflow-hidden">
                        {currentTrack?.coverArt && <img src={currentTrack.coverArt} className="absolute inset-0 w-full h-full object-cover opacity-80 z-0" />}
                        <div className="w-3.5 h-3.5 bg-[#111] rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 border-2 border-white shadow-sm z-20" />
                        {currentTrack && !currentTrack.coverArt && (
                            <div className="text-[7px] md:text-[8px] font-bold text-gray-800 text-center uppercase px-3 leading-tight w-full mt-5 relative z-10">
